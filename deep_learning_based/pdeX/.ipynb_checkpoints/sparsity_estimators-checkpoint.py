...
def get_mask_single(theta,dt,randomized:bool=True,method:str='adaLassoSS'):
...
def Adaptive_Lasso_SS(X,y,
                     ...
                     nI: int = 40,               # number of resamples for the stability selection (B in the paper)
...
#############################################################################
####### original DeepMod sparsity estimator
#############################################################################
def Thresholded_LassoCV(X,y,
                        thr: float = 0.1):

    clf = LassoCV(fit_intercept=False,max_iter=1e4).fit(X,y.ravel())
    coeffs = clf.coef_
    coeffs[np.abs(coeffs) < thr] = 0.0
